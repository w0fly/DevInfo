...
//        val view = inflater.inflate(R.layout.fragment_phone_features, container, false)
    ...
    private fun getDeviceFeatures(): Unit {
        ...
        if (mWifi.isAvailable) {
            tvWifi?.text = "Available"
        } else {
            tvWifi?.text =getString(R.string.not_supported)
        }

        // WIFI Direct
        if (packageManager?.hasSystemFeature(PackageManager.FEATURE_WIFI_DIRECT)!!) {
            tvWifiDirect?.text =getString(R.string.available)
        } else {
            tvWifiDirect?.text =getString(R.string.not_supported)
        }

        // Bluetooth
        if (packageManager?.hasSystemFeature(PackageManager.FEATURE_BLUETOOTH)!!) {
            tvBluetooth?.text =getString(R.string.available)
        } else {
            tvBluetooth?.text =getString(R.string.not_supported)
        }

        // Bluetooth LE
        if (packageManager?.hasSystemFeature(PackageManager.FEATURE_BLUETOOTH_LE)!!) {
            tvBluetoothLE?.text =getString(R.string.available)
        } else {
            tvBluetoothLE?.text =getString(R.string.not_supported)
        }

        // GPS
        if (packageManager?.hasSystemFeature(PackageManager.FEATURE_LOCATION_GPS)!!) {
            tvGPS?.text =getString(R.string.available)
        } else {
            tvGPS?.text =getString(R.string.not_supported)
        }

        // Camera Flash
        if (packageManager?.hasSystemFeature(PackageManager.FEATURE_CAMERA_FLASH)!!) {
            tvCameraFlash?.text =getString(R.string.available)
        } else {
            tvCameraFlash?.text =getString(R.string.not_supported)
        }

        // Camera Front
        if (packageManager?.hasSystemFeature(PackageManager.FEATURE_CAMERA_FRONT)!!) {
            tvCameraFront?.text =getString(R.string.available)
        } else {
            tvCameraFront?.text =getString(R.string.not_supported)
        }

        // Microphone
        if (packageManager?.hasSystemFeature(PackageManager.FEATURE_MICROPHONE)!!) {
            tvMicrophone?.text =getString(R.string.available)
        } else {
            tvMicrophone?.text =getString(R.string.not_supported)
        }

        // NFC
        if (packageManager?.hasSystemFeature(PackageManager.FEATURE_NFC)!!) {
            tvNFC?.text =getString(R.string.available)
        } else {
            tvNFC?.text =getString(R.string.not_supported)
        }

        // USB Host
        if (packageManager?.hasSystemFeature(PackageManager.FEATURE_USB_HOST)!!) {
            tv_usb_host?.text =getString(R.string.available)
        } else {
            tv_usb_host?.text =getString(R.string.not_supported)
        }

        // USB Accessory
        if (packageManager?.hasSystemFeature(PackageManager.FEATURE_USB_ACCESSORY)!!) {
            tv_usb_accessory?.text =getString(R.string.available)
        } else {
            tv_usb_accessory?.text =getString(R.string.not_supported)
        }

        // Multitouch
        if (packageManager?.hasSystemFeature(PackageManager.FEATURE_TOUCHSCREEN_MULTITOUCH)!!) {
            tvMultitouch?.text =getString(R.string.available)
        } else {
            tvMultitouch?.text =getString(R.string.not_supported)
        }

        // Audio low-latency
        if (packageManager?.hasSystemFeature(PackageManager.FEATURE_AUDIO_LOW_LATENCY)!!) {
            tvAudioLowLatency?.text =getString(R.string.available)
        } else {
            tvAudioLowLatency?.text =getString(R.string.not_supported)
        }

        // Audio Output
        if (packageManager?.hasSystemFeature(PackageManager.FEATURE_AUDIO_OUTPUT)!!) {
            tvAudioOutput?.text =getString(R.string.available)
        } else {
            tvAudioOutput?.text =getString(R.string.not_supported)
        }

        // Professional Audio
        if (packageManager?.hasSystemFeature(PackageManager.FEATURE_AUDIO_PRO)!!) {
            tvProfessionalAudio?.text =getString(R.string.available)
        } else {
            tvProfessionalAudio?.text =getString(R.string.not_supported)
        }

        // Consumer IR
        if (packageManager?.hasSystemFeature(PackageManager.FEATURE_CONSUMER_IR)!!) {
            tvConsumerIR?.text =getString(R.string.available)
        } else {
            tvConsumerIR?.text =getString(R.string.not_supported)
        }

        // Gamepad Support
        if (packageManager?.hasSystemFeature(PackageManager.FEATURE_GAMEPAD)!!) {
            tvGamepadSupport?.text =getString(R.string.available)
        } else {
            tvGamepadSupport?.text =getString(R.string.not_supported)
        }

        // HIFI Sensor
        if (packageManager?.hasSystemFeature(PackageManager.FEATURE_HIFI_SENSORS)!!) {
            tvHIFISensor?.text =getString(R.string.available)
        } else {
            tvHIFISensor?.text =getString(R.string.not_supported)
        }

        // Printing
        if (packageManager?.hasSystemFeature(PackageManager.FEATURE_PRINTING)!!) {
            tvPrinting?.text =getString(R.string.available)
        } else {
            tvPrinting?.text =getString(R.string.not_supported)
        }

        // CDMA
        if (packageManager?.hasSystemFeature(PackageManager.FEATURE_TELEPHONY_CDMA)!!) {
            tvCDMA?.text =getString(R.string.available)
        } else {
            tvCDMA?.text =getString(R.string.not_supported)
        }

        // GSM
        if (packageManager?.hasSystemFeature(PackageManager.FEATURE_TELEPHONY_GSM)!!) {
            tvGSM?.text =getString(R.string.available)
        } else {
            tvGSM?.text =getString(R.string.not_supported)
        }

        // Finger-print
        if (packageManager?.hasSystemFeature(PackageManager.FEATURE_FINGERPRINT)!!) {
            tvFingerprint?.text =getString(R.string.available)
        } else {
            tvFingerprint?.text =getString(R.string.not_supported)
        }

        // App Widgets
        if (packageManager?.hasSystemFeature(PackageManager.FEATURE_APP_WIDGETS)!!) {
            tvAppWidgets?.text =getString(R.string.available)
        } else {
            tvAppWidgets?.text =getString(R.string.not_supported)
        }

        // SIP
        if (packageManager?.hasSystemFeature(PackageManager.FEATURE_SIP)!!) {
            tvSIP?.text =getString(R.string.available)
        } else {
            tvSIP?.text =getString(R.string.not_supported)
        }

        // SIP based VOIP
        if (packageManager?.hasSystemFeature(PackageManager.FEATURE_SIP_VOIP)!!) {
            tvSIPBasedVOIP?.text =getString(R.string.available)
        } else {
            tvSIPBasedVOIP?.text =getString(R.string.not_supported)
        }
    }
}