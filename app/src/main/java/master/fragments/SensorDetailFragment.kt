...
//        val view = inflater.inflate(R.layout.fragment_sensors_detail, container, false)
    ...
    private fun displayAccelerometer(event: SensorEvent?) {
        ...
        if (sensorManager?.getDefaultSensor(Sensor.TYPE_ACCELEROMETER)?.type === sensorType
            ...
            tvXAxis?.text = (Html.fromHtml("X: " + formatter.format(x) + getString(R.string.ms) + "<small><sup>2</sup></small>"))
            tvYAxis?.text = (Html.fromHtml("Y: " + formatter.format(y) + getString(R.string.ms) + "<small><sup>2</sup></small>"))
            tvZAxis?.text = (Html.fromHtml("Z: " + formatter.format(z) + getString(R.string.ms) + "<small><sup>2</sup></small>"))
        }
        /*** Magnetic sensors */
            ...
            if (sensorManager?.getDefaultSensor(Sensor.TYPE_MAGNETIC_FIELD)?.type === (sensorType)) {
                tvXAxis?.text = ("X: " + formatter.format(x) + getString(R.string.mu_tesla))
                tvYAxis?.text = ("Y: " + formatter.format(y) + getString(R.string.mu_tesla))
                tvZAxis?.text = ("Z: " + formatter.format(z) + getString(R.string.mu_tesla))
            } else if (sensorManager?.getDefaultSensor(Sensor.TYPE_MAGNETIC_FIELD_UNCALIBRATED)?.type === (sensorType)) {
                tvXAxis?.text = (getString(R.string.geomagnetic_field) + "X: " + formatter.format(x) + getString(R.string.mu_tesla))
                tvYAxis?.text = ("Y: " + formatter.format(y) + getString(R.string.mu_tesla))
                tvZAxis?.text = ("Z: " + formatter.format(z) + getString(R.string.mu_tesla))
            }
        ...
        /*** Gyroscope sensors */
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                tvXAxis?.text = Methods.getSpannableSensorText(mActivity, getString(R.string.angular_speed) + "X: " + formatter.format(x) + getString(R.string.rad))
            }
            tvYAxis?.text = ("Y: " + formatter.format(y) + getString(R.string.rad))
            tvZAxis?.text = ("Z: " + formatter.format(z) + getString(R.string.rad))
        ...
        /*** Rotation sensors */
            ...
            if (sensorManager?.getDefaultSensor(Sensor.TYPE_ORIENTATION)?.type === sensorType) {
                tvXAxis?.text = ("X: " + formatter.format(x) + getString(R.string.degree_icon))
                tvYAxis?.text = ("Y: " + formatter.format(y) + getString(R.string.degree_icon))
                tvZAxis?.text = ("Z: " + formatter.format(z) + getString(R.string.degree_icon))
            } else {
        ...
        /*** Pressure sensor (Barometer) */
            ...
            if (event != null) {
                tvXAxis?.text = (getString(R.string.pressure) + event.values[0] + getString(R.string.hpa))
            }
        }
        /*** Step counter sensor
        Proximity sensor
        Light sensor */
            ...
            if (event != null) {
                if (event.sensor.type == Sensor.TYPE_RELATIVE_HUMIDITY && event.values[0] < sensorManager?.getDefaultSensor(Sensor.TYPE_PROXIMITY)?.maximumRange!!) {
                    tvXAxis?.text = (getString(R.string.proximity_sensor) + event.values[0].toString() + getString(R.string.cm))
                } else {
                    tvXAxis?.text = (getString(R.string.proximity_sensor) + event.values[0].toString() + getString(R.string.cm))
                }

                if (event.sensor.type == Sensor.TYPE_RELATIVE_HUMIDITY) {
                    tvXAxis?.text = (getString(R.string.humidity_sensor) + event.values[0].toString() + getString(R.string.percentage))
                    KeyUtil.KEY_LAST_KNOWN_HUMIDITY = event.values[0]
                }

                if (event.sensor.type == Sensor.TYPE_LIGHT) {
                    tvXAxis?.text = (getString(R.string.illuminance) + event.values[0].toString() + getString(R.string.lx))
                }
                ...
                if (event.sensor.type == Sensor.TYPE_AMBIENT_TEMPERATURE && KeyUtil.KEY_LAST_KNOWN_HUMIDITY != 0f) {
                    ...
                    tvXAxis?.text = (getString(R.string.absolute_humidity_temperature_sensor) + formatter.format(absoluteHumidity) + getString(R.string.percentage))
                    val dewPoint = Methods.calculateDewPoint(temperature, KeyUtil.KEY_LAST_KNOWN_HUMIDITY)
                    tvYAxis?.text = (getString(R.string.due_point_temperature) + formatter.format(dewPoint) + getString(R.string.percentage))
                }

                if (event.sensor.type == Sensor.TYPE_STEP_COUNTER) {
                    tvXAxis?.text = (getString(R.string.steps) + event.values[0].toString())
                }
            }
        }
        /*** Other sensors */
    ...
    private fun displaySensorsDetails(sensorManager: SensorManager) {
        ...
        tvPower?.text = sensorType?.let { sensorManager.getDefaultSensor(it)?.power.toString() + getString(R.string.ma) }
