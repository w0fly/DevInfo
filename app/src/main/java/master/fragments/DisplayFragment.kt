...
class DisplayFragment : BaseFragment(){
    ...
    private fun initToolbar() {
        ...
        tvTitle?.text = getString(R.string.display)
    ...
    private fun getDisplayInfo() {
        ...
        tvPhysicalSize?.text = returnToDecimalPlaces(screenInches).plus(getString(R.string.inches))

        /*** Screen default orientation */
        when {
//            configuration.orientation == Configuration.ORIENTATION_PORTRAIT -> tvScreenDefaultOrientation?.text = getString(R.string.orientation_portrait)
//            configuration.orientation == Configuration.ORIENTATION_LANDSCAPE -> tvScreenDefaultOrientation?.text = getString(R.string.orientation_landscape)
//            configuration.orientation == Configuration.ORIENTATION_UNDEFINED -> tvScreenDefaultOrientation?.text = getString(R.string.orientation_undefined)
        }

        /*** Display screen width and height */
            tvScreenTotalWidth?.text = pWidth.toString().plus(getString(R.string.px))
            tvScreenTotalHeight?.text = pHeight.toString().plus(getString(R.string.px))

        /*** Screen refresh rate */
            tvRefreshRate?.text = display.refreshRate.toString().plus(getString(R.string.fps))
       ...
       /* Handler().postDelayed({
            ...
            gpu.loadOpenGLGles10Info { result -> result.toString()
                tvMaxGpuSize?.text = result.GL_MAX_TEXTURE_SIZE.toString().plus(getString(R.string.x)).plus(result.GL_MAX_TEXTURE_SIZE)
            }
        }, 1000)*/

        /*** Screen Display buckets */
        when {
//            displayMetrics.density == .75f -> tvScreenDensityBucket?.text = getString(R.string.ldpi)
//            displayMetrics.density == 1.0f -> tvScreenDensityBucket?.text = getString(R.string.mdpi)
//            displayMetrics.density == 1.5f -> tvScreenDensityBucket?.text = getString(R.string.hdpi)
//            displayMetrics.density == 2.0f -> tvScreenDensityBucket?.text = getString(R.string.xhdpi)
//            displayMetrics.density == 3.0f -> tvScreenDensityBucket?.text = getString(R.string.xxhdpi)
//            displayMetrics.density == 4.0f -> tvScreenDensityBucket?.text = getString(R.string.xxxhdpi)
        }

        /*** Screen Dpi */
//        tvScreenDPI?.text = displayMetrics.densityDpi.toString().plus(getString(R.string.dpi))
        ...
        tvXDpi?.text = dm.xdpi.toString().plus(getString(R.string.dpi))
        tvYDpi?.text = dm.ydpi.toString().plus(getString(R.string.dpi))


        /*** Screen usable width and height */
        val size = Point()
        display.getSize(size)
        tvScreenUsableWidth?.text = size.x.toString().plus(getString(R.string.px))
        tvScreenUsableHeight?.text = size.y.toString().plus(getString(R.string.px))

//        /*** Screen density independent width and height */
//        tvIndependentWidth?.text = Methods.pxToDp(mActivity, dm.widthPixels).toString().plus(getString(R.string.dp))
//        tvIndependentHeight?.text = Methods.pxToDp(mActivity, dm.heightPixels).toString().plus(getString(R.string.dp))
    }
