...
import com.master.deviceinfo.utilities.*;
...
public class MainActivity extends BaseActivity {
    ...
    public List<DeviceInfoModel> getAppsList() {
        List<DeviceInfoModel> deviceInfoModels = new ArrayList<>();
        ...
        for (ApplicationInfo appInfo : applications) {
            if ((appInfo.flags & ApplicationInfo.FLAG_SYSTEM) == 1) {
                ...
                deviceInfoModels.add(new DeviceInfoModel(1, icon, pm.getApplicationLabel(appInfo).toString(), appInfo.packageName));
            } else {
                // Installed by User
                Drawable icon = pm.getApplicationIcon(appInfo);
                deviceInfoModels.add(new DeviceInfoModel(2, icon, pm.getApplicationLabel(appInfo).toString(), appInfo.packageName));
            }
        }
